...
resultados_var <- aggregate(. ~ output, data = heart_norm[, c('output', variables_interes)], var)
...
Observamos que el dolor torácico (**cp**) más común en nuestros datos es el de Angina atípica (ATA) seguido de la Angina típica (TA). El valor asintomático (ASY) no tiene observaciones.
...
precision_knn <- sum(diag(conf_matrix)) / sum(conf_matrix)
...
ggplot(conf_matrix_df, aes(x = Actual, y = Predicted, size = Count, color = as.factor(Count))) +
  ...
  annotate("text", x = 1.5, y = 2.4, label = paste("Accuracy:", round(precision_knn, 2)))
```
...
Se utilizaron modelos predictivos, como **Random Forest** y **Regresión Logística**, para predecir la variable de salida (output).

Se han construido unos modelos con una precisión superior al 80% siendo el mejor KNN con una precision superior al 84%.

```{r fig.align='center'}
# Crear un data frame con los valores
df <- data.frame(Modelo = c("KNN", "Regresión Logística", "Random Forest"),
                 Precision = c(precision_knn, precision_glm, precision_rf))
